...
const plaintext = Buffer.from("rsa oaep encryption parity");
const ciphertext = crypto.publicEncrypt(publicKey, plaintext);
const decrypted = crypto.privateDecrypt(privateKey, ciphertext);

console.log("cipher is buffer:", Buffer.isBuffer(ciphertext));
console.log("cipher length:", ciphertext.length);
console.log("decrypt text:", decrypted.toString());
console.log("decrypt equals:", Buffer.from(decrypted).toString("hex") === plaintext.toString("hex"));
