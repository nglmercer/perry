import * as crypto from "node:crypto";
import { Buffer } from "node:buffer";

const privateKey = `-----BEGIN PRIVATE KEY-----

/fxuo1/hiam7dCqT/vS1R3mT
-----END PRIVATE KEY-----`;

const publicKey = `-----BEGIN PUBLIC KEY-----

gwIDAQAB
-----END PUBLIC KEY-----`;

const data = Buffer.from("perry rsa signing parity");
const sig = crypto.sign("RSA-SHA256", data, privateKey);

console.log("sig is buffer:", Buffer.isBuffer(sig));
console.log("sig length:", sig.length);
console.log("verify public:", crypto.verify("RSA-SHA256", data, publicKey, sig));
console.log("verify private:", crypto.verify("RSA-SHA256", data, privateKey, sig));
console.log("verify tampered:", crypto.verify("RSA-SHA256", Buffer.from("tampered"), publicKey, sig));
