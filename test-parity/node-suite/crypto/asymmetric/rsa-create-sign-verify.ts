import * as crypto from "node:crypto";
import { Buffer } from "node:buffer";

const privateKey = `-----BEGIN PRIVATE KEY-----

/fxuo1/hiam7dCqT/vS1R3mT
-----END PRIVATE KEY-----`;

const publicKey = `-----BEGIN PUBLIC KEY-----

gwIDAQAB
-----END PUBLIC KEY-----`;

const signer = crypto.createSign("sha256");
signer.update("streaming ");
signer.update(Buffer.from("rsa parity"));
const signature = signer.sign(privateKey);

const verifier = crypto.createVerify("sha256");
verifier.update("streaming ");
verifier.update(Buffer.from("rsa parity"));

console.log("stream sig is buffer:", Buffer.isBuffer(signature));
console.log("stream sig length:", signature.length);
console.log("stream verify:", verifier.verify(publicKey, signature));

const bad = crypto.createVerify("sha256");
bad.update("streaming mismatch");
console.log("stream verify bad:", bad.verify(publicKey, signature));
