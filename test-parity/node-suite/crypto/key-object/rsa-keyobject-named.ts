import { createPrivateKey, createPublicKey, sign, verify, publicEncrypt, privateDecrypt } from "node:crypto";
import { Buffer } from "node:buffer";

const privatePem = `-----BEGIN PRIVATE KEY-----

/fxuo1/hiam7dCqT/vS1R3mT
-----END PRIVATE KEY-----`;

const privateKey = createPrivateKey(privatePem);
const publicKey = createPublicKey(privateKey);
const data = Buffer.from("named key object parity");

const sig = sign("RSA-SHA384", data, privateKey);
const encrypted = publicEncrypt(publicKey, data);

console.log("named keyobject sig len:", sig.length);
console.log("named keyobject verify:", verify("RSA-SHA384", data, publicKey, sig));
console.log("named keyobject decrypt:", privateDecrypt(privateKey, encrypted).toString());
